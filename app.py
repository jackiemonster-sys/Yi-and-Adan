...
# -----------------------------------------------------------------------------
st.set_page_config(
    page_title="☯️ 易經 × 亞當理論 × 機器學習量化旗艦儀表板",
    layout="wide",
...
# 5. UI 介面與動態算力展現
# -----------------------------------------------------------------------------
st.title("☯️ 易經 × 亞當理論 × 機器學習量化旗艦儀表板")
st.caption("即時時空氣場 + ADX 趨勢權重 + 亞當對稱翻轉 + 20日勝率與殘差RMSE驗證")
...
with col_right:
    if run_btn:
        ...
        with st.spinner("算力全開：計算五行氣場、ADX動態權重、亞當對稱與回測勝率..."):
        ...
        if err:
            ...
            if adx_val > 25:
                weight_adam = 0.7
                weight_iching = 0.3
                trend_status = "🔥 趨勢極強 (亞當權重70%)"
            else:
                weight_adam = 0.3
                weight_iching = 0.7
                trend_status = "⚖️ 盤整帶狀 (易經權重70%)"

            hybrid_target = (pred_close * weight_iching) + (adam_target * weight_adam)
            ganzhi_str, day_elem = get_ganzhi_date(calc_date)

            # 多空目標與停損價位
            stop_loss = data["recent_low"] if hybrid_target >= prev_close else data["recent_high"]
            stop_loss_label = "多頭防守停損價(近10日低)" if hybrid_target >= prev_close else "空頭壓力停損價(近10日高)"
            ...
            # 第二列：Plotly 技術分析與預測陰影帶
            df_hist = data["df_history"]
            adam_future_df = data["adam_future_df"]
            
            fig = go.Figure()
            # 1. K線圖
            fig.add_trace(go.Candlestick(x=df_hist.index, open=df_hist["Open"], high=df_hist["High"], low=df_hist["Low"], close=df_hist["Close"], name="歷史K線"))
            fig.add_trace(go.Scatter(x=df_hist.index, y=df_hist["MA5"], line=dict(color="orange", width=1), name="5日線"))
            fig.add_trace(go.Scatter(x=df_hist.index, y=df_hist["MA20"], line=dict(color="blue", width=1), name="20日線"))
            
            # 2. 亞當理論鏡射線
            concat_adam_x = [df_hist.index[-1]] + list(adam_future_df.index)
            concat_adam_y = [prev_close] + list(adam_future_df["Adam_Price"].values)
            fig.add_trace(go.Scatter(x=concat_adam_x, y=concat_adam_y, line=dict(color="purple", width=2, dash="dash"), name="亞當對稱未來軌跡"))

            # 3. 支撐與壓力預估帶 (Region Shade)
            upper_band = max(pred_close, adam_target) + (atr_val * 0.5)
            lower_band = min(pred_close, adam_target) - (atr_val * 0.5)
            
            fig.add_hrect(y0=lower_band, y1=upper_band, fillcolor="rgba(255, 215, 0, 0.15)", line_width=0, annotation_text="預估目標支撐壓力帶", annotation_position="top left")

            fig.update_layout(margin=dict(l=10, r=10, t=10, b=10), xaxis_rangeslider_visible=False, height=380)
            st.plotly_chart(fig, use_container_width=True)

            # 第三列：分頁詳細解讀
            t1, t2, t3, t4 = st.tabs(["📜 老易師量化決策卡", "🔄 20日勝率與遞推日誌", "☯️ 卦象格局結構", "🛡️ 風險與停損評估"])
            ...
            with t1:
                ...
                st.info(yao_info["lucky"] if hybrid_target >= prev_close else yao_info["unlucky"])
            ...
            with t3:
                ...
                st.write(f"**【變卦】收盤**：{raw_res['changed_upper']['symbol']}{raw_res['changed_upper']['name']}上 / {raw_res['changed_lower']['symbol']}{raw_res['changed_lower']['name']}下")

            with t4:
                st.warning(f"⚠️ **亞當對稱失效機制**：若標的價格突破 **{stop_loss:.2f} 元** ({stop_loss_label})，代表對稱型態失效，建議強制停損或反向觀望。")

st.caption("⚠️ 免責聲明：本儀表板結合玄學易經、亞當對稱鏡射與機器學習殘差修正，僅供學術程式開發與研究討論，不構成任何商業買賣建議。")
