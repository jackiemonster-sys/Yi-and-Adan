...
def get_stock_data_and_analysis(stock_code: str, target_date: datetime.date, backtest_days: int = 20, lr: float = 0.3, adam_days: int = 10):
    ...
    for ticker in tickers:
        ...
        if not df.empty and len(df) >= backtest_days + 25:
            ...
            # 🛠️ 關鍵修正：歷史近 N 日高低價，嚴格限定從「實際歷史 df」中計算，絕不包含未來軌跡
    ...
    if run_btn:
        ...
        if err:
            ...
            if adx_val > 25:
                weight_adam = 0.7
                weight_iching = 0.3
                trend_status = f"🔥 趨勢極強 (亞當權重70%)"
            else:
                weight_adam = 0.3
                weight_iching = 0.7
                trend_status = f"⚖️ 盤整帶狀 (易經權重70%)"
            ...
            with t4:
                st.warning(
                    f"⚠️ **亞當對稱失效機制**：當前看{'多' if is_bullish else '空'}。"
                    f"若標的價格{stop_loss_action} ({stop_loss_label})，"
                    f"代表對稱型態與氣場結構已破位失效，建議進行強制停損或反向觀望。"
                )
